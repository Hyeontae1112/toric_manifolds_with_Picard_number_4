    ...
    for i in range(len(K.cpx)):
        ...
        for j in range(i+1, len(K.cpx)):
            ...
            for k in range(len(N)):
                if sp.fraction(sp.together(N[k]))[1] != 1:
                    print('Check dependence of facets %d, %d'%(i, j))
                    return 'Check dependence of facets %d, %d'%(i, j)
            v = N[0]*x[0]
            s=z3.Solver()
            ...
            check = s.check()
            if check == z3.sat:
                return (K.cpx[i], K.cpx[j], M[:, list(np.array(K.cpx[i])-1)], M[:, list(np.array(K.cpx[j])-1)], v, s.model())
...
class Characteristic_Pair(): # [1, ..., n] in K, M = [I_n A]
    def __init__(self, K, M):
        ...
        for i in range(len(self.cpx.cpx)):
            det = sp.expand(sp.det(self.char[:, self.index_cpx[i]]))
    ...
    def subs(self, symbol, replaced):
        if symbol in self._indet:
            ...
            for i in range(len(self._determinant)):
                self._determinant[i] = sp.expand(self._determinant[i].subs(symbol, replaced))
    ...
    def is_fangiving(self):
        if self._is_fangiving is None:
            self._is_fangiving = is_fangiving(self.cpx, self.char, list(self._indet))
